  ...
  try {
    ...
    const { data: tx, error: txErr } = await supabase
      ...
      .insert({
        ...
        note: note || `SPEND by admin ${adminTelegramId} (QR)`,
      })
    ...
    if (updErr) {
      // транзакция уже создана — просто сообщим
      return res.status(200).end(JSON.stringify({ ok: true, tx, targetTelegramId, warning: "QR_USED_MARK_FAILED" }));
    }

    return res.status(200).end(JSON.stringify({ ok: true, tx, targetTelegramId, balanceAfter: balance - amount }));
  } catch (err) {
